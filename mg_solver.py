...
class MGSolver:
    def __init__(self, N, f, omega=2/3, nu=2):
        ...
        self.level_max = int(np.log2(N // 8))
    ...
    def v_cycle(self, N, x, b, level, level_max):
        ...
        ec = np.zeros_like(rc)
        ec = self.v_cycle(N//2, ec, rc, level+1, level_max)
        x += prolong_bilinear(ec, N)
        x = weighted_jacobi(A, x, b, self.omega, self.nu)
        return x

    def solve(self, lmax=None, tol=1e-7, max_cycles=100):
        if lmax is None:
            lmax = self.level_max
        x = np.zeros_like(self.b)
        hist = []
        res0 = np.linalg.norm(self.b - self.A.dot(x))
        ...
        return {'cycles': len(hist), 'hist': hist}
